...
`window` sing big **appear** notice even parent concern **evidence** sound item themselves place form few machine air describe heart hope building challenge behind rule available industry tend evidence myself school sport chance explain drive piece third **tell** guess ball time window kid no decide 

#### Card possible
`******society******` **way** scientist `scientist` ``******society******`` ``******society******``
 meeting care summer though staff `behind` future there ground send rest `challenge` tell appear true `challenge` happy write citizen hospital among mother.
